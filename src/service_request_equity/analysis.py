...
class NeighborhoodAnalyzer:
    ...
    def category_duration_summary(self) -> pd.DataFrame:
        ...
        summary = (
            ...
            .sort_values(["avg_days_open", "total_cases"], ascending=[True, False])
            .reset_index()
        )
        return self._round_float_columns(summary)

    def neighborhood_delay_summary(self) -> pd.DataFrame:
        """Summarize which neighborhoods have the longest average delays."""
        avg_days_open = self.average_days_open()
        summary = (
            self.data.dropna(subset=["Neighborhood", "days_open"])
        ...
        summary["citywide_avg_days_open"] = avg_days_open
        summary["neighborhood_delay_boost"] = (summary["avg_days_open"] - avg_days_open).clip(lower=0)
        return self._round_float_columns(
            summary.sort_values(["avg_days_open", "Neighborhood"], ascending=[False, True]).reset_index(drop=True)
        )
    ...
    def equity_snapshot(self, top_n: int = 5) -> dict[str, Any]:
        ...
        neighborhood_summary = self.neighborhood_delay_summary().head(top_n)
        ...
        return {
            ...
            "neighborhoods_with_highest_avg_delay": neighborhood_summary.to_dict(orient="records"),
            "categories_by_avg_days_open": category_summary.to_dict(orient="records"),
        }
